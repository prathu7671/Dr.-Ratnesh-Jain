...
import { ArrowRight, ExternalLink, Zap, PieChart, TrendingUp } from 'lucide-react';
  ...
  const projects = [
    {
      ...
      impact: [
        { icon: Zap, text: "Reduced R&D costs by 40%" },
        { icon: PieChart, text: "Increased efficacy by 78%" },
        { icon: TrendingUp, text: "Expanded patient eligibility by 35%" }
      ],
    ...
    },
    {
      ...
      impact: [
        { icon: Zap, text: "Saved $4.3M annually" },
        { icon: PieChart, text: "3x manufacturing yield" },
        { icon: TrendingUp, text: "65% faster production" }
      ],
    ...
    },
    {
      ...
      impact: [
        { icon: Zap, text: "Expanded market reach by 40%" },
        { icon: PieChart, text: "Doubled shelf life to 36 months" },
        { icon: TrendingUp, text: "Reduced cold chain costs by 85%" }
      ],
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">
            Featured <span className="text-gradient bg-gradient-to-r from-bio-light-blue via-bio-teal to-bio-blue bg-clip-text text-transparent">Success Stories</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {projects.map((project) => (
            <div 
              key={project.id}
              className="bg-black/40 backdrop-blur-md border border-bio-blue/20 rounded-xl overflow-hidden transition-all duration-300 hover:border-bio-blue/40 hover:shadow-[0_0_20px_rgba(14,165,233,0.2)] hover:-translate-y-1 group"
            >
                ...
                <div className="space-y-2 mb-4">
                  {project.impact.map((item, i) => (
                    <div key={i} className="flex items-center text-sm">
                      <div className="h-5 w-5 rounded-full bg-bio-blue/20 flex items-center justify-center mr-2">
                        <item.icon className="h-3 w-3 text-bio-light-blue" />
                      </div>
                      <span className="text-bio-light-blue">{item.text}</span>
                    </div>
                  ))}
                </div>
